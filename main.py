...
def plot_loss_curve(train_losses, test_losses, fold, dataset_name, save_path):
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(train_losses) + 1), train_losses, label='训练集损失', linewidth=2)
    plt.plot(range(1, len(test_losses) + 1), test_losses, label='测试集损失', linewidth=2)

    plt.title(f'{dataset_name} - 第{fold}折训练/测试损失曲线', fontsize=14)
    ...
    plt.savefig(f"{save_path}/loss_curve_fold{fold}.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for data_name in ['CMI-9589']:
        ...
        for fold in [0,1,2,3,4]:
            ...
            try:
                ...
                plot_loss_curve(train_losses, test_losses, fold, data_name, results_dir)
                print(f"第{fold}折损失曲线已保存至 {results_dir}/loss_curve_fold{fold}.png")
